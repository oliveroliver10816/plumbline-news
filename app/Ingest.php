...
require_once __DIR__ . '/Durable.php';
...
final class Ingest
{
    ...
    public static function run(PDO $p, array $cfg, ?array $only = null): array
    {
        ...
        try {
            ...
            // Mirror the new rows to durable storage. The dyno's disk is wiped on
            // every restart, so anything only in local SQLite is temporary. This
            // is deliberately last and deliberately non-fatal: a mirror that is
            // unreachable must never fail an ingest or take the site down — the
            // next run simply pushes whatever is still missing.
            try {
                if (Durable::enabled($cfg)) {
                    $res['mirrored'] = Durable::push($p, $cfg);
                }
            } catch (\Throwable $e) {
                $res['errors'][] = 'mirror: ' . self::short($e->getMessage());
            }
        } catch (\Throwable $e) {
    ...
    private static function maybePrune(PDO $p, array $cfg, string $dataDir, array &$res): void
    {
        ...
        try {
            ...
            // Retired feeds go too. Everything the page must print beside a story
            // — the newsroom, the licence, the credit line — is read out of the
            // registry by slug, so a story whose feed has been removed can no
            // longer be attributed, and an unattributable story must not be
            // served.
            //
            // The keep-list comes from feedList(), NOT from Feeds::all(): a
            // config-supplied 'feeds' array replaces the built-in registry
            // entirely, and reading the built-in one here would delete every
            // article an operator's own roster had just fetched. feedList() also
            // survives Feeds.php being absent, which the built-in call did not.
            $keep = [];
            foreach (self::feedList($cfg) as $f) {
                $slug = (string) ($f['slug'] ?? '');
                if ($slug !== '') {
                    $keep[] = $slug;
                }
            }
            Db::pruneRetiredSources($p, $keep);
