...
{
    /**
     * Paragraph and line markers used while an item's body is being cleaned.
     * Control characters on purpose: stripHtml() collapses every run of
     * whitespace, so a marker made of whitespace cannot survive it.
     */
    private const PARA_MARK = "\x01\x01";
    private const LINE_MARK = "\x01\x02";
    ...
    /**
     * The most article text one row may hold, and the mark left at the cut.
     * Both are public because app/Render.php has to be able to recognise a
     * truncated body without guessing at a number typed in two places.
     */
    public const BODY_MAX  = 24000;
    public const TRUNCATED = "\u{2026}";

    /**
     * The fullest article text the feed carries, untrimmed.
     *
     * Separate from itemSummary(), which is deliberately short for cards.
     *
     * Every source on this roster was chosen because it publishes the whole
     * article inside the feed — measured median 7,640 characters — so this is
     * where nearly all of a page's text comes from. We render whatever is
     * really in the file and link to the publisher for anything past the cap;
     * we never fetch and republish the full text from their web page.
     *
     * @param array<string,string> $ns
     */
    private static function itemBody(\SimpleXMLElement $it, array $ns, string $kind): string
    {
        ...
        // Paragraph boundaries are marked before the tags are stripped, and the
        // marks are NOT newlines. stripHtml() ends with preg_replace('/\s+/u',
        // ' ') — it is supposed to, because a card summary must be one line — so
        // a "\n\n" put here was flattened again three lines later and every
        // article arrived as a single six-thousand-character paragraph. The
        // marker is therefore a control character, which whitespace collapsing
        // cannot touch, and it is turned back into a blank line afterwards.
        $best = '';
        foreach ($candidates as $raw) {
            $marked = preg_replace('#</(p|div|li|h[1-6]|blockquote)\s*>#i', "$0" . self::PARA_MARK, (string) $raw);
            $marked = preg_replace('#<br\s*/?>#i', self::LINE_MARK, (string) $marked);
            $clean  = self::stripHtml((string) $marked);
            $clean  = str_replace([self::PARA_MARK, self::LINE_MARK], ["\n\n", "\n"], $clean);
            // Any stray control character a publisher smuggled in goes too, so
            // the marks cannot be forged from the outside.
            $clean  = (string) preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F]/u', '', $clean);
        ...
        return self::capBody($best);
    }

    /**
     * Cap the body, on a boundary, and mark the cut.
     *
     * ⚠ THE OLD CAP WAS 6,000 AND IT WAS SIZED FOR THE WRONG ROSTER. It came
     * from the wire-fed build, where the comment "longer than any wire feed
     * supplies" was true — those feeds carry 0-200 characters of body. This
     * roster is the opposite: every source was chosen BECAUSE it publishes the
     * whole article. Measured across all eighteen feeds on 2026-08-22, the
     * median body is 7,640 characters and the 90th percentile is 10,500, so a
     * 6,000 cap silently truncated 77.2% of everything the site carried — and
     * cut it mid-word, with nothing on the page to say so.
     *
     * That is a licence problem as much as a quality one: most of this roster
     * is NoDerivatives, and an article chopped off in the middle of a word is
     * not the work the publisher licensed.
     *
     * BODY_MAX is now 24,000, which clears the 90th percentile more than twice
     * over and leaves the whole three-day corpus at about 1.5x its former body
     * bytes — roughly 4 MB, against a free plan measured in gigabytes. The cap
     * still exists to bound a pathological feed: one Grist item measured 84,387
     * characters because the feed carried the entire page.
     *
     * Where it does still bite, the cut lands on a paragraph, then a sentence,
     * then a word — never mid-word — and ends with an ellipsis, which is what
     * app/Render.php reads to tell the reader the rest is at the publisher.
     */
    private static function capBody(string $body): string
    {
        ...
        $cut = mb_substr($body, 0, self::BODY_MAX);

        // Prefer a paragraph break, then a sentence end, then a word boundary.
        // Each is only accepted if it keeps at least three-quarters of the cap,
        // so a single enormous paragraph is not cut back to nothing.
        $floor = (int) (self::BODY_MAX * 0.75);
        foreach (["\n\n", '. ', ' '] as $mark) {
            $at = mb_strrpos($cut, $mark);
            if ($at !== false && $at >= $floor) {
                $cut = mb_substr($cut, 0, $mark === '. ' ? $at + 1 : $at);
                break;
            }
        }
