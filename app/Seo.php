 ...
 * 4. Nothing here was reported by us, so the structured data says exactly that:
 *    isBasedOn points at the publisher's page, sourceOrganization names the
 *    newsroom that reported it, author is the person who filed it, and no field
 *    is emitted that we cannot substantiate. There is no invented author, no
 *    invented wordCount and no invented image dimensions — Google treats
 *    fabricated structured data as spam, and it would also simply be a lie.
 *
...
final class Seo
{
    ...
    private const STATIC_ROUTES = [
        ...
        ['path' => '/editorial-standards', 'changefreq' => 'yearly', 'priority' => '0.3', 'lastmod' => 'none'],
        ['path' => '/contact',  'changefreq' => 'yearly',  'priority' => '0.3', 'lastmod' => 'none'],
        ['path' => '/privacy',  'changefreq' => 'yearly',  'priority' => '0.2', 'lastmod' => 'none'],
        ['path' => '/terms',    'changefreq' => 'yearly',  'priority' => '0.2', 'lastmod' => 'none'],
    ];

    /**
     * Desks that have a dedicated route of their own instead of a /section/
     * index. There are none: every desk on this site is a section index, which
     * is why the navigation, the sitemap and the per-desk RSS feeds can all be
     * built from one list with no exceptions. Kept as the seam — a future desk
     * with its own template registers here and the sitemap stops emitting the
     * duplicate /section/ URL for it.
     */
    ...
    public static function robotsTxt(array $cfg = []): string
    {
        ...
        // The same fact the footer's standing line and /editorial-standards
        // state, in the one file a crawler reads first. It used to read "full
        // stories stay with their publishers; these pages carry headline,
        // summary and link", which stopped being true the day the article
        // template began rendering the publisher's whole text.
        $out[] = '# Every story here is another newsroom\'s, republished under the licence it grants —';
        $out[] = '# whole where that is permitted, as a short extract where it is not, credited either way.';
        $out[] = '';
        $out[] = 'User-agent: *';
     ...
     * mostly on the slowest feeds on the roster — which are the ones that take
     * longest to earn traffic and need the sitemap most.
     *
