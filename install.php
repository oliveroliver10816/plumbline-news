...
// --- outbound network --------------------------------------------------------
// Probed against THIS edition's own roster rather than a hardcoded list, so
// the check can never report a host the site does not read — and can never
// keep testing one it stopped reading. Three feeds is enough to tell "outbound
// HTTP works" from "outbound HTTP is blocked".
$probe = [];
foreach (array_slice(Feeds::all(), 0, 3) as $f) {
    $probe[(string) $f['name']] = (string) $f['feed'];
}
