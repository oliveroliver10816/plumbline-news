...
/**
 * The desks this edition really runs.
 *
 * ⚠ These used to be the literals 'us', 'international' and 'sports', which is
 * the previous edition's desk list. On this build those are not desks at all,
 * so the sitemap never emitted /section/us, the health report had no 'sports'
 * key, and the assertions that depended on them were testing nothing that
 * exists. Ask the registry instead and the same file tests either edition.
 */
function teb_seo_desk(): string
{
    $home = array_column(TEB\Feeds::homeSections(), 'slug');
...
/**
 * Two feed slugs that are really in this edition's registry.
 *
 * ⚠ These were 'sky-world' and 'dw-world' — the previous edition's feeds. A
 * fail recorded against a slug that is not in the sources table updates zero
 * rows and is silently discarded, so the health assertions here were reading a
 * report about feeds that never existed. The positive control at the end of
 * "the health report writes nothing to the database" is what exposed it: its
 * deliberate write could not move the snapshot hash.
 *
 * @return array{0:string,1:string}
 */
function teb_seo_slugs(): array
{
    $slugs = array_column(TEB\Feeds::all(), 'slug');
    assertTrue(count($slugs) >= 2, 'the registry must carry at least two feeds for this fixture');

    return [$slugs[0], $slugs[1]];
}

/** A registered desk that the fixtures deliberately leave empty. */
function teb_seo_quiet_desk(): string
{
    $used = [teb_seo_desk(), 'world'];
    foreach (array_keys(TEB\Feeds::sections()) as $slug) {
...
function teb_seo_insert_raw(PDO $p, array $row): int
{
    ...
    $columns = [
        ...
        'section'      => (string) ($row['section'] ?? teb_seo_desk()),
...
return [
    ...
    'the sitemap carries the front page, the desks with stories, and every article' => function (): void {
        ...
        assertContains('https://brief.example/section/' . teb_seo_desk(), $locs, 'a desk that has stories');
        assertContains('https://brief.example/section/world', $locs, 'a desk that has stories');
        // Neither edition has a weather desk or a food desk. Both routes 404,
        // and a 404 in a sitemap is the defect this line exists to catch.
        assertNotContains('https://brief.example/weather', $locs, '/weather 404s on this build');
        assertNotContains('https://brief.example/recipes', $locs, '/recipes 404s on this build');
    ...
    'past 5,000 URLs the sitemap becomes an index, and the pages partition the set exactly'
        => function (): void {
            ...
            // The invariant is the partition, not a magic total: every article
            // appears exactly once across the pages, and the standing routes are
            // all there. A hard-coded grand total was brittle — it moved when
            // the standing pages were added and again when the desk list
            // changed, and each time it failed for a reason that had nothing to
            // do with what this test exists to prove.
            $articles = 0;
            $static   = 0;
            foreach (array_keys($seen) as $loc) {
                if (strpos($loc, '/article/') !== false) {
                    $articles++;
                } else {
                    $static++;
                }
            }
            assertSame(5010, $articles, 'every article must appear exactly once across the pages');
            // No '/weather' and no '/recipes': neither edition has a weather
            // desk or a food desk, so both routes 404 and Seo leaves them out.
            // Listing a 404 in a sitemap is the defect.
            foreach (['/', '/sources', '/about',
                      '/editorial-standards', '/contact', '/privacy', '/terms'] as $route) {
                assertTrue(
                    isset($seen['https://brief.example' . rtrim($route, '/')])
                        || isset($seen['https://brief.example' . $route]),
                    'the paged sitemap dropped ' . $route
                );
            }
            // Seven standing routes — the front page, /sources and the five
            // AdSense pages — plus at least one desk that has stories on it.
            // It was nine while /weather was a standing route of its own; that
            // page and its desk are gone, so the floor is eight.
            assertGreaterThanOrEqual(8, $static, 'the standing routes and the live desks');
    ...
    'the health report answers every question it exists to answer' => function (): void {
        ...
        Db::recordIngestRun($p, [
            ...
            'inserted'     => 31,
            'skipped'      => 90,
            'errors'       => [teb_seo_slugs()[0] . ': HTTP 503'],
        ]);
        ...
        assertContains(teb_seo_slugs()[0] . ': HTTP 503', $report['ingest']['last_run']['errors']);
        ...
        assertSame(1, $report['sections'][teb_seo_desk()]['last_24h']);
        assertSame(2, $report['sections']['world']['last_7_days']);
        assertSame(0, $report['sections'][teb_seo_quiet_desk()]['last_7_days'], 'a quiet desk is visible, not absent');
    ...
    'a failing feed is reported with the error it actually returned' => function (): void {
        ...
        [$slugA, $slugB] = teb_seo_slugs();
        Db::recordFeedResult($p, $slugA, false, 'cURL error 28: Operation timed out after 12001 ms');
        for ($i = 0; $i < 8; $i++) {
            Db::recordFeedResult($p, $slugB, false, 'HTTP 403 from the publisher');
        }
        ...
        assertArrayHasKey($slugA, $bySlug);
        assertSame(1, $bySlug[$slugA]['fail_count']);
        assertFalse($bySlug[$slugA]['parked']);
        assertContains('Operation timed out', $bySlug[$slugA]['last_error']);

        assertArrayHasKey($slugB, $bySlug);
        assertSame(8, $bySlug[$slugB]['fail_count']);
        assertTrue($bySlug[$slugB]['parked']);
        assertContains('HTTP 403', $bySlug[$slugB]['last_error']);
    ...
    'a story the pruner will not delete is in the sitemap, however old its dateline'
        => function (): void {
            ...
            Db::insertArticles($p, [teb_seo_row([
                ...
                'source_name'  => 'Budget Bytes',
                'section'      => 'health',
                'url'          => 'https://kffhealthnews.org/old-but-live',
                'title'        => 'A report dated sixty days ago that arrived in the feed today',
            ...
            assertContains('https://brief.example/section/health', $locs, 'and its desk is listed too');

            // The desk page must carry the dateline it actually has, not nothing.
            $xml = Seo::sitemap($p, $cfg);
            assertMatches(
                '#<loc>https://brief\.example/section/health</loc>\s*<lastmod>#',
                $xml,
    ...
    'the health report writes nothing to the database' => function (): void {
        ...
        Db::upsertSources($p, Feeds::all());
        teb_seo_seed($p);
        Db::recordFeedResult($p, teb_seo_slugs()[0], false, 'HTTP 503');
        Db::recordIngestRun($p, [
            'started_at'  => teb_seo_now() - 40000,
            'finished_at' => teb_seo_now() - 20000,
            'run_mode'    => 'cron',
        ...
        Db::recordFeedResult($p, teb_seo_slugs()[0], false, 'HTTP 500');
    ...
    'a per-desk feed exists only for a desk that exists' => function (): void {
        // Seo::rss() ignores a section name it does not recognise and returns the
        // whole site feed. Left alone at the routing layer that meant every string
        // anyone typed — /feed/sports.xml, /feed/weather.xml, /feed/anything.xml —
        // answered 200 with the same fifty items: an unbounded space of duplicate
        // URLs on one document. The router refuses the name instead.
        teb_require_app('Images', 'Placeholder', 'Compose', 'Rotate', 'Ingest', 'Xml', 'Durable', 'Pages', 'Router');
        ...
        $desk = teb_seo_desk();
        $ok   = TEB\Router::dispatch($p, $cfg, '/feed/' . $desk . '.xml', []);
        assertSame(200, (int) $ok['status'], 'a real desk must still have its own feed');
        assertContains('rss', (string) $ok['headers']['Content-Type']);
        teb_seo_wellformed((string) $ok['body'], 'desk feed');

        foreach (['sports', 'weather', 'recipes', 'not-a-desk-at-all'] as $bogus) {
            $res = TEB\Router::dispatch($p, $cfg, '/feed/' . $bogus . '.xml', []);
            assertSame(404, (int) $res['status'], "/feed/$bogus.xml must not serve the whole site feed");
        }

        // The site-wide feed is unaffected — it is a different route.
        assertSame(200, (int) TEB\Router::dispatch($p, $cfg, '/feed.xml', [])['status']);
    },
