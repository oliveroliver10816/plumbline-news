...
');
    ...
    'a source that leaves the registry stops being credited on the site' => function (): void {
        // The defect this pins was live in both databases: a store that outlives
        // an edition keeps every source it has ever had marked active, and the
        // /sources page and the footer's credit column are built from exactly
        // that column — so the site went on naming ABC News, ESPN, Fox News and
        // a recipe blog as sources it had never read a word from.
        $p = teb_db_fresh();

        $before = [
            ['slug' => 'abc-us',    'name' => 'ABC News', 'feed' => 'https://feeds.abcnews.com/abcnews/usheadlines', 'section' => 'us'],
            ['slug' => 'espn-news', 'name' => 'ESPN',     'feed' => 'https://espn.test/rss',  'section' => 'sports'],
            ['slug' => 'grist',     'name' => 'Grist',    'feed' => 'https://grist.org/feed/', 'section' => 'environment'],
        ];
        Db::upsertSources($p, $before);
        assertCount(3, Db::sources($p, true), 'the fixture starts with three live sources');

        // The roster changes: two newsrooms are dropped.
        Db::upsertSources($p, [
            ['slug' => 'grist',      'name' => 'Grist',           'feed' => 'https://grist.org/feed/', 'section' => 'environment'],
            ['slug' => 'kff-health', 'name' => 'KFF Health News', 'feed' => 'https://kffhealthnews.org/feed/', 'section' => 'health'],
        ]);

        $live = array_column(Db::sources($p, true), 'slug');
        sort($live);
        assertSame(['grist', 'kff-health'], $live, 'a retired source must not stay in the credit list');

        // Deactivated, NOT deleted: the row keeps its id and its fail history so
        // a feed that comes back does not start again from nothing.
        $abc = Db::sourceBySlug($p, 'abc-us');
        assertTrue(is_array($abc), 'the retired row is kept, not dropped');
        assertSame(0, $abc['active']);

        // Re-registering it does NOT flip it live again on the spot, and that is
        // deliberate: 'active' is also the flag ingest uses to park a feed after
        // eight failures, and the older guarantee — "re-registering a feed must
        // not silently un-park it" — has to keep holding. One successful fetch
        // is the only thing that revives a source, which is exactly the rule the
        // parking test above pins.
        Db::upsertSources($p, $before);
        assertSame(0, Db::sourceBySlug($p, 'abc-us')['active'], 're-registering must not un-park');
        Db::recordFeedResult($p, 'abc-us', true, '', teb_now());
        assertSame(1, Db::sourceBySlug($p, 'abc-us')['active'], 'one good fetch brings a returning source back');
    },
