...
require_once dirname(__DIR__) . '/app/Paths.php';
require_once dirname(__DIR__) . '/app/Feeds.php';
require_once dirname(__DIR__) . '/app/Placeholder.php';
...
return [
    ...
    'every desk this edition runs has its own placeholder ink' => function (): void {
        // ⚠ THIS IS THE MOST-REPEATED GRAPHIC ON THE SITE, not an edge case.
        // Only NASA licenses its photographs to us, so nearly every card on
        // nearly every page is drawn by Placeholder. The palette it shipped
        // with was keyed to the previous edition's desks — 'us',
        // 'international', 'sports', 'recipes' — so three of this edition's
        // desks, two of which lead the front page, fell through to a single
        // hardcoded default and drew in exactly the same colour as politics.
        $palette = (new ReflectionClass('TEB\Placeholder'))->getConstant('PALETTE');
        assertTrue(is_array($palette) && $palette !== [], 'the palette must exist');

        $missing = [];
        foreach (array_keys(Feeds::sections()) as $slug) {
            if (!array_key_exists($slug, $palette)) {
                $missing[] = $slug;
            }
        }
        assertSame([], $missing, 'these desks would be drawn on a fallback colour');

        $stale = [];
        foreach (array_keys($palette) as $slug) {
            if (Feeds::section($slug) === null) {
                $stale[] = $slug;
            }
        }
        assertSame([], $stale, 'the palette names desks this edition does not run');
    },

    'a desk with no palette entry still gets its own colour, not politics own' => function (): void {
        // The old default meant any desk added to the registry later drew in the
        // house colour, silently. Unknown desks are now spread deterministically
        // across the same ladder.
        $cfg   = ['site' => ['name' => 'Fixture', 'short_name' => 'FX']];
        $paper = strtolower((string) (new ReflectionClass('TEB\Placeholder'))->getConstant('PAPER'));

        // The ink is every colour in the drawing that is not the paper.
        $inkOf = static function (string $desk) use ($cfg, $paper): string {
            preg_match_all('/#[0-9A-Fa-f]{6}/', Placeholder::svg($desk, 0, $cfg), $m);
            $inks = array_values(array_unique(array_filter(
                array_map('strtolower', $m[0]),
                static fn(string $c): bool => $c !== $paper
            )));
            assertCount(1, $inks, 'a placeholder should be exactly one ink on one paper: ' . json_encode($inks));

            return $inks[0];
        };

        $seen = [];
        foreach (['alpha-desk', 'bravo-desk', 'charlie-desk', 'delta-desk', 'echo-desk', 'foxtrot-desk'] as $s) {
            $seen[$s] = $inkOf($s);
        }
        assertTrue(count(array_unique($seen)) > 1, 'unknown desks all drew the same colour: ' . json_encode($seen));

        // ...and the answer never moves, or the browser cache churns.
        assertSame($seen['alpha-desk'], $inkOf('alpha-desk'), 'the colour for a given desk must be stable');
    },

    'the placeholder paper is this edition own, not the other edition one' => function (): void {
        // The two mastheads must not share the most-repeated graphic on the
        // page. The paper is read off this site's --well design token.
        $paper = (new ReflectionClass('TEB\Placeholder'))->getConstant('PAPER');
        assertMatches('/^#[0-9A-Fa-f]{6}$/', (string) $paper);

        $css = (string) file_get_contents(teb_root() . '/assets/css/site.css');
        assertContains(strtolower((string) $paper), strtolower($css),
            'the placeholder paper must be a colour this stylesheet actually uses');
    },
];
