    ...
    return [
        ...
        'subdirectory /teb, no rewrite' => [
            ...
            'urls'     => [
                ...
                '/sources'             => '/teb/index.php?r=/sources',
            ],
            'asset'    => '/teb/assets/css/site.css',
        ...
        'nested /a/b, no rewrite' => [
            ...
            'urls'     => [
                ...
                '/sources'             => '/a/b/index.php?r=/sources',
            ],
            'asset'    => '/a/b/assets/css/site.css',
...
return [
    ...
    'config.php parses and returns every top-level section' => function (): void {
        ...
        foreach (['site', 'db', 'ingest', 'compose', 'ads', 'cache'] as $key) {
    ...
    'config defaults are the ones the spec fixed' => function (): void {
        ...
        assertSame('Plumbline News', $c['site']['name']);
        assertSame('PN', $c['site']['short_name']);
        assertSame('plumblinenews.com', $c['site']['domain']);
        assertSame('America/New_York', $c['site']['timezone']);
        assertSame('sqlite', $c['db']['driver']);
        assertFalse($c['ads']['enabled']);

        assertSame(1, $c['compose']['finance_max_on_home']);
        assertSame(['hero', 'politics', 'environment', 'education'], $c['compose']['finance_blocked_blocks']);
        assertSame(4, $c['compose']['hero_sub_count']);
        assertSame(3, $c['compose']['per_source_cap_per_block']);
        assertSame(14, $c['compose']['ticker_count']);

        assertTrue($c['ingest']['auto_on_empty']);
        assertSame(20, $c['ingest']['stale_after_minutes']);
        // 60, not 3. These newsrooms publish over weeks rather than hours, and
        // a three-day window measured out 300 of 359 stored articles — config.php
        // says so at the setting itself. Pinned here so it cannot drift back.
        assertSame(60, $c['ingest']['retention_days']);

        // Every desk the money quota names must be a desk this edition really
        // runs, or the guard rail is protecting nothing. This is the assertion
        // that would have caught the shipped default naming 'us' on a site with
        // no U.S. desk.
        $desks = array_column(Feeds::homeSections(), 'slug');
        foreach ($c['compose']['finance_blocked_blocks'] as $b) {
            assertTrue($b === 'hero' || in_array($b, $desks, true), $b . ' is not a desk on this edition');
        }
    },
    ...
    'Config::get reads dotted paths and honours the default' => function (): void {
        ...
        assertSame('Plumbline News', Config::get('site.name'));
        assertSame(1, Config::get('compose.finance_max_on_home'));
    ...
    'a partial config.php is merged over the defaults, not swapped in' => function (): void {
        ...
        assertSame(1, $c['compose']['finance_max_on_home']);     // default kept
    ...
    'ads.slots is replaced by the client, never merged' => function (): void {
        // The ad slots are the one list Config replaces wholesale instead of
        // merging: a slot the client DELETED has to stay deleted, and a deep
        // merge would quietly resurrect it on the next page view.
    ...
    'a mistyped config is repaired instead of taking the site down' => function (): void {
        ...
        assertSame(
            ['hero', 'politics', 'environment', 'education'],
    ...
    'a whole section typed as a scalar keeps the shipped section, and never fatals' => function (): void {
        ...
        $broken = [
            ...
            "'cache' as a float"   => "<?php return ['cache' => 1.5];",
        ...
        foreach ($broken as $label => $src) {
            ...
            foreach (['site', 'db', 'ingest', 'compose', 'ads', 'cache'] as $section) {
            ...
            assertSame(14, $c['compose']['ticker_count'], $label . ': an unusable section falls back whole');
        }
    ...
    'currentRoute() normalises every shape a request can arrive in' => function (): void {
        ...
        $cases = [
            ...
            ['/teb/index.php',             '',                 '/'],
            ['/teb//section//us',          '',                 '/section/us'],
            ['/teb/sources?page=2',        'page=2',           '/sources'],
            ['/teb/index.php?r=/sources',  'r=/sources',       '/sources'],
            ['/teb/index.php?r=sources',   'r=sources',        '/sources'],
            ['/teb/index.php?r=/teb/sources', 'r=/teb/sources', '/sources'],
    ...
    'the feed registry is the size and shape the build expects' => function (): void {
        ...
        // The roster is the eighteen full-text feeds measured in docs/RECON.md.
        // The bound is tight on purpose: this list is small BECAUSE almost no
        ...
        assertSame(25, count($feeds), 'the roster is the measured twenty-five, no more and no fewer');
    ...
    'the registry is feeds only — there is no JSON service left to park' => function (): void {
        // This edition has no recipe desk and no weather desk: there is no
        // recipe feed on the measured roster and the forecast page was removed
        // with its route, so neither has anything to register. What used to be
        // asserted here — three recipe feeds, two JSON endpoints — would now be
        // a demand to invent sources, which is the opposite of the rule this
        // build exists to keep.
        assertSame([], Feeds::bySection('recipes'), 'no recipe feed is registered on this edition');
        assertSame([], Feeds::bySection('weather'), 'no weather feed is registered on this edition');
        assertFalse(method_exists(Feeds::class, 'services'), 'the JSON service registry went with the weather page');

        // A JSON endpoint in the RSS registry would be parsed as XML, fail, and
        // be parked after eight runs. The registry is XML feeds and nothing else.
        foreach (Feeds::all() as $f) {
            assertNotContains('themealdb', $f['feed']);
            assertNotContains('open-meteo', $f['feed']);
            assertNotContains('api.weather.gov', $f['feed']);
            assertMatches('#^https://#', $f['feed'], $f['slug']);
        }
    },
    ...
    'every feed URL in RECON.md is registered' => function (): void {
        ...
        assertSame(25, count($wanted), 'RECON.md must document every feed on the roster');
        ...
        // The other direction, which was never checked: a feed added to the
        // registry but never measured and written down would have passed the
        // test above without appearing in it at all.
        $undocumented = [];
        foreach (array_column(Feeds::all(), 'feed') as $mine) {
            if (!in_array($mine, $wanted, true)) {
                $undocumented[] = $mine;
            }
        }
        assertSame([], $undocumented, 'every registered feed must be measured and recorded in RECON.md');
    },

    'no feed on the do-not-add list is registered' => function (): void {
        // ⚠ THIS LIST WAS THE PREVIOUS EDITION'S and it could not fail here: it
        // banned recipe sites and wire hubs that nothing on this roster was ever
        // going to reach for. The real do-not-add list is the one in FEEDS.md
        // and docs/RECON.md — publishers that were fetched, measured, and found
        // to carry a teaser instead of an article. Those are the ones somebody
        // would plausibly re-add, because their names look respectable.
        $banned = [
            // measured stubs: median body, in characters, in the comment
            'arstechnica.com',        // 1,021
            'quantamagazine.org',     //   408
            'insideclimatenews.org',  //   354
            'sciencedaily.com',       //   343
            'phys.org',               //   307
            'defense.gov',            //   228
            'themarshallproject.org', //   138
            'noaa.gov',               //    91
            'energy.gov',             //    83
            'texastribune.org',       //     3
            'undark.org',             //     0
            // dead or blocking from this box
            'nsf.gov', 'usgs.gov', 'wikinews', 'nih.gov', 'stateline.org', 'hbr.org', 'cdc.gov',
        ];

        // Positive control: the matcher really does catch a banned host.
        assertTrue(
            strpos('https://arstechnica.com/feed/', $banned[0]) !== false,
            'the substring test is doing work'
        );
        foreach (Feeds::all() as $f) {
    ...
    'the front page section order is the one the spec fixes' => function (): void {
        assertSame(
            ['us-news', 'politics', 'environment', 'education', 'health', 'financial', 'world', 'business', 'technology', 'science', 'culture'],
            array_column(Feeds::homeSections(), 'slug')
        );
        assertContains('business', Feeds::financeSections());
        assertNotContains('politics', Feeds::financeSections(), 'the lead desk is not a money desk');

        foreach (Feeds::sections() as $slug => $meta) {
            assertSame($slug, $meta['slug']);
            assertTrue($meta['label'] !== '', $slug . ' needs a label');
        }
        assertSame('Politics', Feeds::sections()['politics']['label']);
    ...
    'bySlug and bySection answer honestly' => function (): void {
        ...
        foreach (Feeds::bySection('politics') as $f) {
            assertSame('politics', $f['section']);
        }
        assertTrue(count(Feeds::bySection('politics')) > 0, 'the lead desk has feeds, so the loop above ran');
