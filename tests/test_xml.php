...
return [
    ...
    'the body cap clears this roster instead of truncating most of it' => function (): void {
        // ⚠ THE CAP USED TO BE 6,000 AND IT WAS THE WRONG NUMBER. It was carried
        // over from the wire-fed build, whose feeds supply 0-200 characters of
        // body, so it never bit there. Every source on THIS roster was chosen
        // because it publishes the whole article: measured across all eighteen
        // feeds, the median body is 7,640 characters and the 90th percentile is
        // 10,500. At 6,000 the site was silently cutting 77.2% of everything it
        // carried, mid-word, with nothing on the page to say so — which on a
        // NoDerivatives roster is a licence problem, not only a quality one.
        assertGreaterThanOrEqual(11000, Xml::BODY_MAX, 'the cap must clear the measured 90th percentile');
        assertLessThanOrEqual(60000, Xml::BODY_MAX, 'but a cap that never bites is not a cap');
    },

    'a body at the cap is cut on a boundary and marked, never mid-word' => function (): void {
        $cap = Xml::BODY_MAX;
        $long = str_repeat('Sentences of ordinary length, each one ending properly. ', (int) ceil($cap / 20));
        assertTrue(mb_strlen($long) > $cap, 'positive control: the fixture really is over the cap');

        $out = teb_xml_cap($long);

        assertTrue(mb_strlen($out) <= $cap + 1, 'the cap is respected (plus the one-character mark)');
        assertSame(Xml::TRUNCATED, mb_substr($out, -1), 'a cut body is marked, so the page can say it was cut');

        // The point of the boundary search: the last thing before the mark is a
        // whole word, not half of one.
        $before = mb_substr($out, 0, -1);
        assertTrue(
            mb_substr($long, 0, mb_strlen($before)) === $before,
            'the kept text must be a genuine prefix of the article'
        );
        assertMatches('/[.\s]$|[a-z]$/iu', $before, 'the cut landed inside a word');
        $nextChar = mb_substr($long, mb_strlen($before), 1);
        assertTrue(
            $nextChar === ' ' || $nextChar === '' || $before[strlen($before) - 1] === '.',
            'the character after the cut proves the boundary: got ' . json_encode($nextChar)
        );
    },

    'a body under the cap is returned untouched and unmarked' => function (): void {
        $short = 'A complete short article that is nowhere near the limit.';
        assertSame($short, teb_xml_cap($short), 'a normal article must not be altered at all');
        assertNotSame(Xml::TRUNCATED, mb_substr(teb_xml_cap($short), -1), 'and must not be marked as cut');

        // Exactly at the cap is not over it.
        $exact = str_repeat('a', Xml::BODY_MAX);
        assertSame($exact, teb_xml_cap($exact), 'a body of exactly BODY_MAX is whole, not cut');
    },

    'a pathological single-word body is still bounded' => function (): void {
        // One Grist item measured 84,387 characters because the feed carried the
        // whole page. There may be no boundary to cut on at all; the cap still
        // has to hold, and the mark still has to be there.
        $blob = str_repeat('x', Xml::BODY_MAX * 3);
        $out  = teb_xml_cap($blob);

        assertTrue(mb_strlen($out) <= Xml::BODY_MAX + 1, 'no boundary must not mean no cap');
        assertSame(Xml::TRUNCATED, mb_substr($out, -1));
    },
];

/** Reach the private cap so the boundary rule is tested directly, not inferred. */
function teb_xml_cap(string $body): string
{
    $m = new ReflectionMethod('TEB\Xml', 'capBody');
